...
function processDataRender(response)
{
    ...
    for(var i=0; i<data.length; i++) 
    {
        ...
        if(can_delete == true && row.bank_status == 0)
        {
        ...
        if(row.bank_status == -1 && can_approve == true)
        {
        ...
        {
            row.bank_status = "<span class='badge badge-secondary'>Pending Approval</span>";
        }
        else if(row.bank_status == 1)
        {
...
function renderData() 
{
    ...
    cols = ['chboxes', 'bank_description', 'bank_code', 'created_by', 'bank_status', 'createdat', 'actions'];
    
    var filters = getFilters();
    filters.bank_status = 'pending';
    var columns = [];
...
{
    $('#view-bank-approval-modal').modal('show');
    loadingBlockUI('#view-bank-approval-modal');
    ...
    $.get(WEBSITE_URL + "banks/get/" + id, postdata, function(response){
        if(response.status == "success")
        {
            ...
            bank_status = "<span class='badge badge-secondary'>Unknown</span>";
            if(data.bank_status == 0){
                bank_status = "<span class='badge badge-danger'>Inactive</span>";
            }
            else if(data.bank_status == -1){
                bank_status = "<span class='badge badge-secondary'>Pending Approval</span>";
            }
            else if(data.bank_status == 1){
                bank_status = "<span class='badge badge-success'>Approved</span>";
            }
            // Load the data into the form
            $("#td-bank_description").html(data.bank_description);
            $("#td-bank_code").html(data.bank_code);
            $("#td_initiated_by").html(data.firstname + ' ' + data.lastname);
            $("#td_approval_status").html(bank_status);
            $("#td_createdat").html(dateToReadableTime(data.createdat));
        }
        else
        {
            $('#view-bank-approval-modal').modal('hide');
    ...
    .fail(function(xhr, statusText, errorThrown) {
        ...
        ajaxFailureResponse(xhr.status, '#view-bank-approval-modal');
    });
}
...
function approveRecords()
{
    swal({
        ...
        if (result.value)
        {
            ...
            $.post(WEBSITE_URL + "banks/approve", postdata, function(response){
                if(response.status == "success")
                {
                    // Refresh the list
                    renderData();
...
// Method to approve a single bank selection
function approve()
{
    swal({
        title: 'Are you sure?',
