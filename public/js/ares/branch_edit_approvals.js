...
function processDataRender(response)
{
    ...
    for(var i=0; i<data.length; i++) 
    {
        ...
        row.branch_description = _data.branch_description
        row.branch_code = _data.branch_code
...
function renderData() 
{
    ...
    cols = ['chboxes', 'record_id', 'branch_description', 'branch_code', 'created_by', 'approval_status', 'createdat', 'actions'];
    
    var filters = getFilters();
    filters.module = 'BRANCHES';
...
function loadData(id)
{
    ...
    $.get(WEBSITE_URL + "data_edits/get/" + id, postdata, function(response){
        if(response.status == "success")
        {
            ...
            color = '';
            if(data_new.branch_description != data_old.branch_description)
                color = 'text-danger';

            $("#td-branch_description-new").html('<span class="'+color+'">' + data_new.branch_description + '</span>');
            $("#td-branch_description-old").html('<span class="'+color+'">' + data_old.branch_description + '</span>');

            color = '';
            if(data_new.branch_code != data_old.branch_code)
                color = 'text-danger';

            $("#td-branch_code-new").html('<span class="'+color+'">' + data_new.branch_code + '</span>');
            $("#td-branch_code-old").html('<span class="'+color+'">' + data_old.branch_code + '</span>');

            color = '';
            if(data_new.branch_address != data_old.branch_address)
                color = 'text-danger';

            $("#td-branch_address-new").html('<span class="'+color+'">' + data_new.branch_address + '</span>');
            $("#td-branch_address-old").html('<span class="'+color+'">' + data_old.branch_address + '</span>');
