...
function processDataRender(response)
{
    ...
    for(var i=0; i<data.length; i++) 
    {
        ...
        row.currency_description = _data.currency_description
        row.currency_code = _data.currency_code
...
function renderData() 
{
    ...
    cols = ['chboxes', 'record_id', 'currency_description', 'currency_code', 'created_by', 'approval_status', 'createdat', 'actions'];
    
    var filters = getFilters();
    filters.module = 'CURRENCIES';
...
function loadData(id)
{
    ...
    $.get(WEBSITE_URL + "data_edits/get/" + id, postdata, function(response){
        if(response.status == "success")
        {
            ...
            color = '';
            if(data_new.currency_description != data_old.currency_description)
                color = 'text-danger';

            $("#td-currency_description-new").html('<span class="'+color+'">' + data_new.currency_description + '</span>');
            $("#td-currency_description-old").html('<span class="'+color+'">' + data_old.currency_description + '</span>');

            color = '';
            if(data_new.currency_code != data_old.currency_code)
                color = 'text-danger';

            $("#td-currency_code-new").html('<span class="'+color+'">' + data_new.currency_code + '</span>');
            $("#td-currency_code-old").html('<span class="'+color+'">' + data_old.currency_code + '</span>');

            color = '';
            if(data_new.currency_iso != data_old.currency_iso)
                color = 'text-danger';

            $("#td-currency_iso-new").html('<span class="'+color+'">' + data_new.currency_iso + '</span>');
            $("#td-currency_iso-old").html('<span class="'+color+'">' + data_old.currency_iso + '</span>');

            color = '';
            if(data_new.country_iso != data_old.country_iso)
                color = 'text-danger';

            $("#td-country_iso-new").html('<span class="'+color+'">' + data_new.country_iso + '</span>');
            $("#td-country_iso-old").html('<span class="'+color+'">' + data_old.country_iso + '</span>');
